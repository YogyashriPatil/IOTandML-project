...
function mapBackendToUI(b) {
  ...
  // FIX: stage "current" is based on harvestedDaysAgo mapped to a stage index
  // harvestedDaysAgo=0 → D0 (Fresh), =1 → D1 (Ripening), etc.
  // We clamp to valid stage indices
  const currentStageIndex = Math.min(harvestedDaysAgo, stagesRaw.length - 1);
  ...
  const stages = stagesRaw.map((label, i) => ({
    ...
    warn:    i === stagesRaw.length - 2 && i !== currentStageIndex,
    danger:  i === stagesRaw.length - 1,
    alert:   i === 1 && (b.prediction?.chemicalProb ?? 0) > 40,
  }));
...
// FIX: SensorPill now receives the sensor alert ranges from backend directly
// so ALERT only fires when the actual sensor value is truly out of normal range
function SensorPill({ data }) {
  const ok = data.value >= data.safe[0] && data.value <= data.safe[1];
  return (
    <div className="sensor-pill" style={{
...
// FIX: confidence badge — show actual confidence, not hardcoded label
function ConfidenceBadge({ value }) {
  const color = value >= 80 ? T.safe : value >= 60 ? "#f59e0b" : T.danger;
  ...
  return (
    <div style={{ textAlign: "center", flexShrink: 0 }}>
      ...
      <div style={{ fontSize: 10, color: T.textMuted, marginTop: 2, letterSpacing: "0.05em" }}>CONFIDENCE</div>
      <div style={{ fontSize: 9, color, marginTop: 3, fontWeight: 600 }}>{label}</div>
    </div>
  );
}
...
export default function App() {
  ...
  const riskColor = (risk) => {
    if (risk === "High")   return { color: T.danger,  bg: "rgba(248,113,113,0.12)",  bd: "rgba(248,113,113,0.25)" };
    if (risk === "Medium") return { color: "#f59e0b",  bg: "rgba(245,158,11,0.12)",   bd: "rgba(245,158,11,0.25)" };
    return                        { color: T.safe,     bg: "rgba(0,245,160,0.12)",    bd: "rgba(0,245,160,0.25)" };
  };
  ...
  return (
    <div style={{ fontFamily: "'Plus Jakarta Sans', 'Outfit', sans-serif", background: T.bg, minHeight: "100vh", color: T.textPri, overflowX: "hidden" }}>
      ...
      <div style={{ maxWidth: 1000, margin: "0 auto", padding: "32px 20px", position: "relative", zIndex: 1 }}>
        ...
        {/* STEP 5: RESULTS */}
        {step === 5 && result && (() => {
          const rc = riskColor(result.prediction.risk);
          return (
            <div>
              ...
              {/* Row 2: Verdict Banner — FIX: risk badge color is now from riskColor() not hardcoded */}
              <div className="fu" style={{
                ...glass({ padding: "18px 22px", marginBottom: 16, borderRadius: 18 }),
                border: `1.5px solid ${sBd}`, background: sBg,
                display: "flex", alignItems: "center", gap: 16,
                ...
                <div style={{ flex: 1, minWidth: 200 }}>
                  ...
                  <div style={{ fontSize: 12, color: T.textSec, lineHeight: 1.6 }}>
                    {edible
                      ? "All sensor readings within natural ripening thresholds."
                      : result.validity.storageAdvice}
                  </div>
                  {result.prediction.flags.length > 0 && (
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 10 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, flexShrink: 0 }}>
                  <ConfidenceBadge value={result.prediction.confidence} />
                  {/* FIX: Risk badge now uses riskColor() for proper color coding */}
                  <div style={{
              ...
              <div className="fu grid-3" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14, animationDelay: "160ms" }}>
                ...
                <div className="card">
                  ...
                  <ProbBar label="Natural Ripening"  value={result.prediction.naturalProb}  color={T.safe}    delay={300} />
                  <ProbBar label="Chemical Ripening" value={result.prediction.chemicalProb} color="#f87171"  delay={480} />
                  {/* FIX: Show condition label below bars */}
                  <div style={{ marginTop: 12, padding: "8px 12px", borderRadius: 10, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
                    <div style={{ fontSize: 9, color: T.textMuted, letterSpacing: "0.06em", marginBottom: 3 }}>CONDITION</div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: T.textSec }}>
                      {result.prediction.label}
                    </div>
                    <div style={{ fontSize: 9, color: T.textMuted, marginTop: 3 }}>
                      {result.prediction.chemicalProb > 28
                        ? `Chemical indicators: ${result.prediction.chemicalProb.toFixed(1)}%`
                        : "No chemical indicators detected"}
                    </div>
                  </div>
                </div>
